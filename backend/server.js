...
// Parse FRONTEND_URLS as comma-separated list
...
app.post('/api/auth/change-password', authenticateToken, async (req, res) => {
  try {
    ...
    await logAudit(req.user.username, 'UPDATE', 'admins', req.user.id, { action: 'password_change' }, { action: 'password_changed' }, req.ip);
...
app.put('/api/admin/church-info', authenticateToken, requireSuperAdmin, async (req, res) => {
  try {
    ...
    const {
      ...
      facebook, nav_title, about_content,
    ...
    const result = await pool.query(`
      UPDATE church_info SET
        ...
        facebook = $17, show_members_link = $18, nav_title = $19, about_content = $20,
        field_label_pastor = $21, field_label_address = $22, field_label_phone = $23,
        field_label_email = $24, field_label_website = $25, field_label_facebook = $26,
        updated_at = NOW()
      WHERE id = 1
      RETURNING *
    `, [name, address, phone, email, website, logo_url,
        ...
        nav_title || 'Churches Directory', about_content || '',
